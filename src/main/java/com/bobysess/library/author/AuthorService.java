...
public class AuthorService {
  ...
  private void validateAuthor(Author author) {
    ...
    if (author.getLastName() == null || author.getLastName().isBlank()) {
      throw new InvalidAuthorException(
          InvalidAuthorException.Reason.MISSING_LAST_NAME, "Last name is required");
    }
    if (author.getBirthDate() != null && author.getBirthDate().isAfter(LocalDate.now())) {
      throw new InvalidAuthorException(
          InvalidAuthorException.Reason.INVALID_BIRTH_DATE, "Birth date cannot be in the future");
    }
    if (author.getDeathDate() != null && author.getDeathDate().isAfter(LocalDate.now())) {
      throw new InvalidAuthorException(
          InvalidAuthorException.Reason.INVALID_DEATH_DATE, "Death date cannot be in the future");
    }
    if (author.getBirthDate() != null
        && author.getDeathDate() != null
